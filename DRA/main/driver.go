...
func NewDriver(initialPool []string, nodeName, driverName string) *Driver {
	...
	// Start periodic stale claim detection and cleanup
	go driver.startStaleClaimMonitoring()

	return driver
}

// generateUniqueIP detects and returns the existing IP address assigned to eth1 interface
// Returns empty string if no IP is detected on eth1
func (d *Driver) generateUniqueIP() string {
	log.Printf("[DRASecondaryNIC] Generating IP for node: %s", d.nodeName)

	// Try to detect existing IP on eth1 interface
	existingIP := d.detectExistingIPOnInterface("eth1")
	if existingIP != "" {
		log.Printf("[DRASecondaryNIC] Found existing IP %s on eth1 interface", existingIP)
		return existingIP
	}

	// No existing IP found on eth1
	log.Printf("[DRASecondaryNIC] WARNING: No IP address found on eth1 interface for node %s", d.nodeName)
	return ""
}

// detectExistingIPOnInterface detects existing IP address on the specified interface
func (d *Driver) detectExistingIPOnInterface(interfaceName string) string {
	log.Printf("[DRASecondaryNIC] Detecting existing IP on interface %s", interfaceName)

	// Run ip addr show command to get interface information
	cmd := exec.Command("/sbin/ip", "addr", "show", interfaceName)
	output, err := cmd.CombinedOutput()
	if err != nil {
		log.Printf("[DRASecondaryNIC] Failed to get IP info for interface %s: %v", interfaceName, err)
		return ""
	}

	outputStr := string(output)
	log.Printf("[DRASecondaryNIC] Interface %s info: %s", interfaceName, outputStr)

	// Parse the output to extract IP addresses
	// Look for lines containing "inet " (IPv4 addresses)
	lines := strings.Split(outputStr, "\n")
	for _, line := range lines {
		line = strings.TrimSpace(line)
		if strings.HasPrefix(line, "inet ") && !strings.Contains(line, "127.0.0.1") {
			// Extract IP address with CIDR notation
			// Example line: "inet 10.9.255.4/24 brd 10.9.255.255 scope global eth1"
			parts := strings.Fields(line)
			if len(parts) >= 2 {
				ipWithCIDR := parts[1]
				log.Printf("[DRASecondaryNIC] Found IP address %s on interface %s", ipWithCIDR, interfaceName)
				return ipWithCIDR
			}
		}
	}

	log.Printf("[DRASecondaryNIC] No IP address found on interface %s", interfaceName)
	return ""
}

// moveInterfaceWithIPCommand moves a network interface to a target namespace using ip command
func (d *Driver) moveInterfaceWithIPCommand(interfaceName, targetNamespace string) error {
	log.Printf("[DRASecondaryNIC] Moving interface %s to namespace %s using ip command", interfaceName, targetNamespace)

	// Use /sbin/ip to move the interface to the target namespace
	cmd := exec.Command("/sbin/ip", "link", "set", interfaceName, "netns", targetNamespace)
	output, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to move interface %s to namespace %s: %v, output: %s",
			interfaceName, targetNamespace, err, string(output))
	}

	log.Printf("[DRASecondaryNIC] Successfully moved interface %s to namespace %s", interfaceName, targetNamespace)
	return nil
}

// moveInterfaceBackToHost attempts to move a network interface back to the host namespace
func (d *Driver) moveInterfaceBackToHost(interfaceName string) error {
	log.Printf("[DRASecondaryNIC] Attempting to move interface %s back to host namespace", interfaceName)

	// First, check if the interface is already in the host namespace
	cmd := exec.Command("/sbin/ip", "link", "show", interfaceName)
	_, err := cmd.CombinedOutput()
	if err == nil {
		log.Printf("[DRASecondaryNIC] Interface %s is already in host namespace", interfaceName)
		return nil
	}

	// Interface not in host namespace, try to find it in network namespaces and move it back
	// List all network namespaces
	cmd = exec.Command("/sbin/ip", "netns", "list")
	nsOutput, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to list network namespaces: %v", err)
	}

	// Check each namespace for the interface
	...
	for _, line := range nsLines {
		...
		// Extract namespace name (before any parentheses)
		parts := strings.Fields(line)
		if len(parts) == 0 {
			continue
		}
		...
		// Check if the interface exists in this namespace
		cmd = exec.Command("/sbin/ip", "netns", "exec", nsName, "/sbin/ip", "link", "show", interfaceName)
		_, err = cmd.CombinedOutput()
		if err == nil {
			// Found the interface in this namespace, move it back to host
			log.Printf("[DRASecondaryNIC] Found interface %s in namespace %s, moving back to host", interfaceName, nsName)
			cmd = exec.Command("/sbin/ip", "netns", "exec", nsName, "/sbin/ip", "link", "set", interfaceName, "netns", "1")
			moveOutput, moveErr := cmd.CombinedOutput()
			if moveErr != nil {
				return fmt.Errorf("failed to move interface %s from namespace %s to host: %v, output: %s",
					interfaceName, nsName, moveErr, string(moveOutput))
			}
			log.Printf("[DRASecondaryNIC] Successfully moved interface %s back to host namespace", interfaceName)
			return nil
		}
	}

	// Interface not found in any namespace
	log.Printf("[DRASecondaryNIC] Interface %s not found in any network namespace", interfaceName)
	return nil
}

// configureIPWithIPCommand configures IP address on interface in target namespace using ip command
func (d *Driver) configureIPWithIPCommand(interfaceName, targetNamespace, ipAddress string) error {
	log.Printf("[DRASecondaryNIC] Configuring IP %s on interface %s in namespace %s", ipAddress, interfaceName, targetNamespace)

	// Add IP address to the interface in the target namespace
	cmd := exec.Command("/sbin/ip", "netns", "exec", targetNamespace, "/sbin/ip", "addr", "add", ipAddress, "dev", interfaceName)
	output, err := cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to configure IP %s on interface %s in namespace %s: %v, output: %s",
			ipAddress, interfaceName, targetNamespace, err, string(output))
	}

	// Bring the interface up in the target namespace
	cmd = exec.Command("/sbin/ip", "netns", "exec", targetNamespace, "/sbin/ip", "link", "set", interfaceName, "up")
	output, err = cmd.CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to bring up interface %s in namespace %s: %v, output: %s",
			interfaceName, targetNamespace, err, string(output))
	}

	// Configure routing for multi-homed networking (return traffic via same interface)
	err = d.configureMultiHomedRouting(interfaceName, targetNamespace, ipAddress)
	if err != nil {
		log.Printf("[DRASecondaryNIC] Warning: Failed to configure multi-homed routing: %v", err)
		// Don't fail the entire configuration if routing fails - interface is still usable
	}

	log.Printf("[DRASecondaryNIC] Successfully configured IP %s on interface %s in namespace %s", ipAddress, interfaceName, targetNamespace)
	return nil
}

// configureMultiHomedRouting configures source-based routing for multi-homed networking
// This ensures traffic that comes in via the secondary interface (eth1) routes back via the same interface
func (d *Driver) configureMultiHomedRouting(interfaceName, targetNamespace, ipAddress string) error {
	log.Printf("[DRASecondaryNIC] Configuring multi-homed routing for %s (%s) in namespace %s", interfaceName, ipAddress, targetNamespace)

	// Parse IP address to get the host IP (without CIDR)
	ip, ipNet, err := net.ParseCIDR(ipAddress)
	if err != nil {
		return fmt.Errorf("failed to parse IP address %s: %v", ipAddress, err)
	}
	...
	// Use numeric table ID instead of string name (200 for eth1)
	tableID := "200"

	// 1. Add policy rule: traffic FROM this IP should use the interface table
	// Use sh instead of bash for broader container compatibility
	cmd := exec.Command("/sbin/ip", "netns", "exec", targetNamespace, "/sbin/ip", "rule", "add",
		"from", ip.String(), "table", tableID)
	output, err := cmd.CombinedOutput()
	if err != nil {
		log.Printf("[DRASecondaryNIC] Warning: Failed to add policy rule from %s table %s: %v, output: %s",
			ip.String(), tableID, err, string(output))
	} else {
		log.Printf("[DRASecondaryNIC] Added rule: from %s table %s", ip.String(), tableID)
	}

	// 2. Add default route in the interface table
	cmd = exec.Command("/sbin/ip", "netns", "exec", targetNamespace, "/sbin/ip", "route", "add",
		"default", "via", gateway.String(), "dev", interfaceName, "table", tableID)
	output, err = cmd.CombinedOutput()
	if err != nil {
		log.Printf("[DRASecondaryNIC] Warning: Failed to add default route via %s dev %s table %s: %v, output: %s",
			gateway.String(), interfaceName, tableID, err, string(output))
	} else {
		log.Printf("[DRASecondaryNIC] Added route: default via %s dev %s table %s", gateway.String(), interfaceName, tableID)
	}

	// 3. Force route cache flush to ensure new rules take effect
	cmd = exec.Command("/sbin/ip", "netns", "exec", targetNamespace, "/sbin/ip", "route", "flush", "cache")
	output, err = cmd.CombinedOutput()
	if err != nil {
		log.Printf("[DRASecondaryNIC] Warning: Failed to flush route cache: %v, output: %s", err, string(output))
	} else {
		log.Printf("[DRASecondaryNIC] Route cache flushed successfully")
	}

	log.Printf("[DRASecondaryNIC] Multi-homed routing configuration completed for %s", interfaceName)
	return nil
}
...
func (d *Driver) PrepareResourceClaims(
	...
	log.Printf("[DRASecondaryNIC] *** PrepareResourceClaims called with %d claims ***", len(claims))
	for i, claim := range claims {
		log.Printf("[DRASecondaryNIC] PrepareResourceClaims[%d]: UID=%s, Name=%s, Namespace=%s", i, claim.UID, claim.Name, claim.Namespace)
	}
...
func (d *Driver) UnprepareResourceClaims(
	...
	log.Printf("[DRASecondaryNIC] *** UnprepareResourceClaims called with %d claims ***", len(claims))
	for i, claim := range claims {
		log.Printf("[DRASecondaryNIC] UnprepareResourceClaims[%d]: UID=%s, Name=%s, Namespace=%s", i, claim.UID, claim.Name, claim.Namespace)
	}
	...
	for _, claim := range claims {
		...
		if nicHost, ok := d.claims[key]; ok {
			// Try to move the network interface back to host namespace if it's not already there
			err := d.moveInterfaceBackToHost(nicHost)
			if err != nil {
				log.Printf("[DRASecondaryNIC] Warning: failed to move interface %s back to host: %v", nicHost, err)
			}
...
func (d *Driver) ConfigureNetwork(ctx context.Context, req *pb.ConfigureNetworkRequest) (*pb.ConfigureNetworkResponse, error) {
	...
	// Extract namespace name from the provided network namespace path
	networkNamespacePath := req.GetNetworkNamespacePath()
	if networkNamespacePath == "" {
		return &pb.ConfigureNetworkResponse{
			Success:      false,
			ErrorMessage: "Network namespace path not provided",
			Interfaces:   []*pb.ConfiguredInterface{},
		}, nil
	}

	// Extract just the namespace name from the full path
	// e.g., "/var/run/netns/cni-f87a7b5f-8ab1-5ec2-5992-80e872c90bca" -> "cni-f87a7b5f-8ab1-5ec2-5992-80e872c90bca"
	networkNamespaceName := networkNamespacePath
	...
	log.Printf("[DRASecondaryNIC] Network namespace path: %s, extracted name: %s", networkNamespacePath, networkNamespaceName)

	// Perform actual network interface movement and configuration
	// Move the interface to the target namespace
	err = d.moveInterfaceWithIPCommand(cfg.NICName, networkNamespaceName)
	if err != nil {
		log.Printf("[DRASecondaryNIC] Failed to move interface: %v", err)
		return &pb.ConfigureNetworkResponse{
			Success:      false,
			ErrorMessage: fmt.Sprintf("Failed to move interface %s: %v", cfg.NICName, err),
			Interfaces:   []*pb.ConfiguredInterface{},
		}, nil
	}

	// Configure IP address on the interface in the target namespace
	err = d.configureIPWithIPCommand(cfg.NICName, networkNamespaceName, cfg.IP)
	if err != nil {
		log.Printf("[DRASecondaryNIC] Failed to configure IP: %v", err)
		return &pb.ConfigureNetworkResponse{
			Success:      false,
			ErrorMessage: fmt.Sprintf("Failed to configure IP %s on interface %s: %v", cfg.IP, cfg.NICName, err),
			Interfaces:   []*pb.ConfiguredInterface{},
		}, nil
	}

	log.Printf("[DRASecondaryNIC] Successfully completed network interface configuration")
	log.Printf("[DRASecondaryNIC] Interface %s moved to namespace %s with IP %s", cfg.NICName, networkNamespaceName, cfg.IP)
...
// detectAndCleanupStaleClaims identifies claims that might be stuck due to kubelet issues
// and attempts to clean them up by checking if pods still exist and interfaces are available
func (d *Driver) detectAndCleanupStaleClaims() {
	d.mu.Lock()
	defer d.mu.Unlock()

	log.Printf("[DRASecondaryNIC] Checking for stale claims that might need cleanup")

	var staleClaims []string
	for claimUID, hostNIC := range d.claims {
		// Check if the network interface is back in the host namespace
		// This indicates the pod has terminated but kubelet hasn't called UnprepareResourceClaims
		if d.isInterfaceInHostNamespace(hostNIC) {
			log.Printf("[DRASecondaryNIC] Detected stale claim %s for interface %s (interface returned to host namespace)", claimUID, hostNIC)

			// Additional validation: check if this claim has been allocated for a long time
			// and should be considered for cleanup due to kubelet reconciler issues
			staleClaims = append(staleClaims, claimUID)
		}
	}

	// Clean up stale claims
	if len(staleClaims) > 0 {
		log.Printf("[DRASecondaryNIC] Cleaning up %d stale claims", len(staleClaims))
		for _, claimUID := range staleClaims {
			if hostNIC, exists := d.claims[claimUID]; exists {
				// Remove CDI device
				devName := "nic-" + claimUID
				_ = removeCDIDevice(devName)

				// Return interface to free pool
				d.releaseNICLocked(hostNIC)
				delete(d.claims, claimUID)
				log.Printf("[DRASecondaryNIC] Cleaned up stale claim %s for interface %s", claimUID, hostNIC)
			}
		}
		// Update resource availability after cleanup
		d.UpdateResourceAvailability()
	}
}

// isInterfaceInHostNamespace checks if a network interface is currently in the host namespace
func (d *Driver) isInterfaceInHostNamespace(interfaceName string) bool {
	cmd := exec.Command("/sbin/ip", "link", "show", interfaceName)
	_, err := cmd.CombinedOutput()
	return err == nil // If command succeeds, interface is in host namespace
}

// startStaleClaimMonitoring runs periodic checks for stale claims every 2 minutes
func (d *Driver) startStaleClaimMonitoring() {
	ticker := time.NewTicker(2 * time.Minute)
	defer ticker.Stop()

	log.Printf("[DRASecondaryNIC] Started stale claim monitoring (checking every 2 minutes)")

	for {
		select {
		case <-ticker.C:
			d.detectAndCleanupStaleClaims()
		}
	}
}
